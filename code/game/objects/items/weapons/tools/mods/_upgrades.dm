...
/datum/component/item_upgrade/proc/apply(obj/item/A, mob/living/user)
	if(user)
		...
		if(!I.use_tool(user = user, target =  A, base_time = WORKTIME_FAST, required_quality = null, fail_chance = FAILCHANCE_ZERO, required_stat = STAT_INT, forced_sound = WORKSOUND_WRENCHING))
			return FALSE
			...
			if(IU.removable == MOD_INTEGRAL)
				if(istype(upgrade_loc, /obj/item/gun/projectile/automatic/modular))
					...
					if(MG.loaded.len || MG.ammo_magazine || MG.chambered)
						...
						return TRUE
			if(C.use_tool(user = user, target =  upgrade_loc, base_time = IU.removal_time, required_quality = QUALITY_SCREW_DRIVING, fail_chance = IU.removal_difficulty, required_stat = STAT_INT))
