    ...
    if (active)
        for (var/mob/living/carbon/M in range(10, src))
            if (is_excelsior(M))
                ...
                M.stats.addTempStat(STAT_END, STAT_LEVEL_ADEPT, 30 SECONDS, "ex_boombox")
                M.stats.addTempStat(STAT_AGI, STAT_LEVEL_BASIC, 30 SECONDS, "ex_boombox")
            else
                to_chat(M, SPAN_WARNING("You hear some stupid distracting communistic propaganda."))
                M.stats.addTempStat(STAT_AGI, -STAT_LEVEL_BASIC, 30 SECONDS, "ex_boombox_m")
