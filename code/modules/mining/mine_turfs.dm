	...
	switch(tool_type)
		...
		if(QUALITY_EXCAVATION)
			...
			if(excavation_amount)
				...
				if(I.use_tool(user, src, WORKTIME_NORMAL, tool_type, FAILCHANCE_NORMAL, required_stat = STAT_LCK))
...
/turf/simulated/floor/asteroid/attackby(obj/item/I, mob/user)
	...
	if(QUALITY_DIGGING in I.tool_qualities)
		if (dug)
			...
			return
		if(I.use_tool(user, src, WORKTIME_FAST, QUALITY_DIGGING, FAILCHANCE_EASY, required_stat = STAT_STR))
