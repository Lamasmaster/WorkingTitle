...
/datum/computer_file/program/access_decrypter/Topic(href, href_list)
	...
	if(href_list["PRG_execute"])
		...
		running = TRUE
		operator_skill = get_operator_skill(usr, STAT_LCK)
		if(ntnet_global.intrusion_detection_enabled && !prob(get_sneak_chance()))
...
/datum/nano_module/program/access_decrypter/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = NANOUI_FOCUS, var/datum/nano_topic_state/state = GLOB.default_state)
	if(!ntnet_global)
		return
